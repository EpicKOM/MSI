...
class SaintMartinDheresData(db.Model):
    ...
    def get_data_status(cls):
        try:
            ...
            app.logger.exception(
                "[SaintMartinDheresData - get_data_status] - Erreur lors de la récupération du status des données de la table "
    ...
    def get_current_weather_data(cls):
        try:
            ...
            base_weather_data = {"update_datetime": last_record.date_time.strftime("%d/%m/%Y à %H:%M"),
                                 ...
                                 "uv": last_record.uv if last_record.uv is not None else None,
                                 }
    ...
    def get_daily_extremes(cls):
        try:
            ...
            app.logger.exception(
                "[SaintMartinDheresData - get_daily_extremes] - Erreur lors de la récupération des températures et des rafales "
                "extrêmes du jour.")
